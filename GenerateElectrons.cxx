...
)
{
  ...
  // PARAMETERS
  int n_hits = 5;
  Double_t recoil_threshold = threshold; // detection threshold in keV
  TString electron_output_filename
    = Form("sim/e_%s_q_%.3f_m_%.3f_%s_%ss_%ihits_%.0fkeV",
	   detector.Data(),charge_mcp,mass_mcp,horn.Data(),meson.Data(),n_hits,threshold);
  ...
  auto electron_recoil = [ mass_mcp,
			   ...
			   ]( TLorentzVector mom )
			 {
			   ...
			   for ( int i = 0; i < n_hits; i++ ) {
			     ...
			     Er = DetectorInteraction(mom.E(),mass_mcp,charge_mcp,recoil_threshold)/1000.;
			     if ( Er == 0 ) {
			       break;
			     }
