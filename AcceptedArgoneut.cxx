...
Double_t AcceptedArgoneut(TString fstr)
{
  ...
  // loops over events
  int events = -1;
  Double_t value = 0; // result stored here
  ...
  while ( reader.Next() ) {
    ...
    */
    if ( abs(atan2(Mom->Px(),Mom->Pz())) < xdev && abs(atan2(Mom->Py(),Mom->Pz())) < ydev ) {
      //value+= (*weight_meson)*(*weight_decay);
      //value+= (*weight_meson);
      value++;
    }
  ...
  cout << Form("value: %.3f\nPOT normalized value: %.3f\n(old) xsec: %.7f\nsum weight decays: %.3f\nsum weight meson: %.3f",
	       value,(1e20/500000.)*value/(sum_weight_decay*0.5),*xsec,sum_weight_decay/2.,sum_weight_meson/2.) << endl;

  Double_t result = (1e20/500000.)*(truexsec)*value; // result uses truexsec
  cout << "geometrical acceptance " << value/(events) << endl;
